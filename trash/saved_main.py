...
# 2) Сделать класс Menu
# 3) Сделать класс Game
...
def MENU(text):
    ...
    t1 = FONT.render(text, True, (255, 0, 0))
    win.blit(t1, (int(window_width/2), int(window_height/2)))

    t2 = FONT.render('Нажмите любую клавишу для продолжения', True, (255, 0, 0))
    win.blit(t2, (int(window_width/2), int(window_height/2)+100))
...
player = Player(x=700, y=750, height=80, width=80, health=80, frames_paths=frms) 


# объекты
#player2 = Abstract_object(500,590,50,130,1000,[])
bg_image_1 = Picture('bg/bg_{}_{}.png'.format(window_width,window_height),0) 
bg_image_2 = Picture('bg/bg_{}_{}.png'.format(window_width,window_height),-window_height-1) 
...
while GAME_RUN:
    ...
    ############################
    if DEBUG:
        debug_information(win, {'x':player.x, 'y': player.y,'size': pygame.display.get_desktop_sizes(), 'entities': Abstract_object.get_objects_count(), 'bg1':bg_image_1.y, 'bg2': bg_image_2.y})
